...
export const createUser=async(req:Request,res:Response):Promise<void>=>{
    try {
        ...
        if(!username || !email || !password || !first_name || !last_name){
            res.status(400).json({error:'Missing/Invalid parameters, the following parameters are required: username,email, password, first_name, last_name'})
            return
        }
        const user:User={
            username,
...
export const UpdateUser = async (req: Request, res: Response): Promise<void> => {
  ...
  export const loginUser = async (req: Request, res: Response): Promise<void> => {
    try {
      const user_id = await userModel.authenticate(req.body.username as string, req.body.password as string)
      if (user_id) {
